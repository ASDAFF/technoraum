...
if($_SESSION["DISCOUNT"])
{
	foreach($arResult["GRID"]["ROWS"] as $item)
	{
		...
		if(!($item["data"]["PROPERTY_OLD_PRICE_VAL_VALUE"] > 0))
		{
			$arResult["GRID"]["ROWS"][$row_id]["data"]["PROPERTY_OLD_PRICE_VAL_VALUE"] = 0;
			$arResult["GRID"]["ROWS"][$row_id]["columns"]["PROPERTY_OLD_PRICE_VAL_VALUE"][0]["value"] = 0;
			
			$_SESSION["SUMM"] = $_SESSION["SUMM"] + (($item["data"]["PRICE"] - $item["data"]["PRICE"] / 100 * $_SESSION["DISCOUNT"]) * $item["data"]["QUANTITY"]);
				...
				?>
					<script>
						$(document).ready(function()
						{
							var i = <?=$i?>;
							var j = 0;
							$(".bx-soa-item-tr.bx-soa-basket-info").each(function()
							{
								if(j == i)
								{
									$(this).children(".bx-soa-item-td").filter(":nth-child(3)").children(".bx-soa-item-td-text").children("span").text("Скидка: <?=$_SESSION["DISCOUNT"]."%"?>");
									
									var old_price = $(this).children(".bx-soa-item-td").filter(":last-child").children(".bx-soa-item-td-text").children(".bx-price.all").text();
									var new_price = old_price.split("руб.");
									new_price = new_price[0];
									new_price = new_price.replace(" " , "");
									new_price = new_price * 1;
									new_price = new_price - new_price / 100 * <?=$_SESSION["DISCOUNT"]?>;
									new_price = new_price + "";
									
									new_price = new_price.split("").reverse().join("");
									var result = [];
									var count = new_price.length-1;
									
									for(k=0,l=0,m=0;k<=count;k++,l++,m++)
									{
										if(l == 3)
										{
											result[k] = " ";
											result[k+1] = new_price[m];
											k++;
											count++;
											l = 0;
										}
										else
											result[k] = new_price[m];
									}
									result = result.reverse().join("");
									new_price = result;
									$(this).children(".bx-soa-item-td").filter(":last-child").children(".bx-soa-item-td-text").children(".bx-price.all").html(new_price + " руб.<br><span style='text-decoration: line-through;color:#a9a7a7;'>" + old_price + "</span>");
								}
								j++;
							});
						});
				...
				?>
					<script>
						$(document).ready(function()
						{
							...
							{
								if(j == i)
									$(this).children(".bx-soa-item-td").filter(":nth-child(2)").children(".bx-soa-item-td-text").children("span").text(" - ");
								j++;
							});
						});
					</script>
		...
		{
			$arResult["GRID"]["ROWS"][$row_id]["data"]["PROPERTY_OLD_PRICE_VAL_VALUE"] = " - ";
			$arResult["GRID"]["ROWS"][$row_id]["columns"]["PROPERTY_OLD_PRICE_VAL_VALUE"][0]["value"] = " - ";
			
			$_SESSION["SUMM"] = $_SESSION["SUMM"] + $item["data"]["PRICE"] * $item["data"]["QUANTITY"];
			?>
				<script>
					$(document).ready(function()
					{
						...
						{
							if(j == i)
								$(this).children(".bx-soa-item-td").filter(":nth-child(3)").children(".bx-soa-item-td-text").children("span").text(" - ");
							j++;
						});
					});
				</script>
	...
	if($_SESSION["P_COUNT"] > 0)
	{
		...
		$(document).ready(function()
		{
			var summ = "<?=$_SESSION["SUMM"]?>";
			var old_price = $(".bx-soa-cart-total").filter(":first").children(".bx-soa-cart-total-line").filter(":nth-child(3)").children(".bx-soa-cart-d").text();
			
			summ = summ.split("").reverse().join("");
			var result = [];
			var count = summ.length-1;
			for(k=0,l=0,m=0;k<=count;k++,l++,m++)
			{
				if(l == 3)
				{
			...
			$(".bx-soa-cart-total").children(".bx-soa-cart-total-line").filter(":nth-child(3)").children(".bx-soa-cart-d").html(summ + " руб.<br><span style='text-decoration: line-through;color:#a9a7a7;'>" + old_price + "</span>");
			
			$(".bx-soa-item-tr.bx-soa-basket-info").each(function()
			{
				$(this).children(".bx-soa-item-td").filter(":nth-child(2)").detach();
			});
		});
		</script>
		<?
...
else
{
	...
	?>
	<form action="<?=$APPLICATION->GetCurPage();?>" method="POST" name="ORDER_FORM" id="bx-soa-order-form" class="create_order" enctype="multipart/form-data">
		<?
		...
		<div id="bx-soa-order" class="row bx-<?=$arParams['TEMPLATE_THEME']?>" style="opacity: 0">
			...
			<div class="col-sm-9 bx-soa">
				...
				<? if (!isset($arParams['BASKET_POSITION']) || $arParams['BASKET_POSITION'] === 'before'): ?>
					<!--	BASKET ITEMS BLOCK	-->
					<div id="bx-soa-basket" data-visited="true" class="bx-soa-section bx-active bx-selected">
						<div class="bx-soa-section-title-container">
							<h2 class="bx-soa-section-title col-sm-9">
								<span class="bx-soa-section-title-count"></span>Товары в заказе
							</h2>
							<div class="col-xs-12 col-sm-3 text-right">
								<a href="javascript:void(0)" class="bx-soa-editstep">изменить</a>
							</div>
						</div>	
						<div class="bx-soa-section-content container-fluid"></div>
					</div>
				...
				<div id="bx-soa-orderSave">
					...
					<input type="submit" style="margin: 10px 0" class="pull-right btn btn-default btn-lg hidden-xs" value="Оформить заказ" />
				</div>
