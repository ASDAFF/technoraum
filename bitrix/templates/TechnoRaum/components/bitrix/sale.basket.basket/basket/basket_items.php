...
?>
<style>
.gift_block p:first-child{color: #FF0000;font-size: 14px;margin: 0;padding: 0}
.gift_block ul{font-size: 13px;padding-left: 30px;font-weight: 100}
</style>
							...
							?>
								...
								<td class="item">
									<h2 class="bx_ordercart_itemtitle">
										...
										<? 
											$line = CIBlockElement::GetProperty(8, $arItem["PRODUCT_ID"], array("sort" => "asc"), Array("CODE" => "GIFT"));
											$data = $line->Fetch();
											if($data["VALUE"])
											{
												$full_summ = 0;
												...
												if($ar_res = $res->GetNext())
												{
													...
													$gift_items[] = array("NAME" => $ar_res["NAME"] , "PRICE" => $price["PRICE"]);
													$full_summ+=$price["PRICE"];
												}
												...
												?>
													...
													<p>+ Подарки на сумму <?=number_format($full_summ , 0 , " " , " ")?> руб.:</p>
													<ul>
														<?
															foreach($gift_items as $gitem)
															{
																?><li><?=$gitem["NAME"]?> - <?=number_format($gitem["PRICE"] , 0 , " ", " ")?> &#8381;</li><?
															}
														?>
													</ul>
													</div>
