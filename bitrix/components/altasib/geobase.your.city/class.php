...
class CAltasibGeobaseYourCityThemes extends CBitrixComponent
{
	...
	public function Generate($color = false, $theme = false, $scheme = false, $tmplPath, $strStyle)
	{
		$filename = $_SERVER['DOCUMENT_ROOT'].$tmplPath.'/themes/theme_'.md5($theme.'_'.$color.'_'.$scheme).'.css';
		if(file_exists($filename)){
			return;
		}
		$dirName = $_SERVER['DOCUMENT_ROOT'].$tmplPath.'/themes/';
		...
		if(empty($strStyle))
			$strStyle = GetMessage("AGB_YC_STYLE_GENERATE");
		...
		$ret = file_put_contents($filename, $style);
	}
}