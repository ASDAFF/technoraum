...
class CSeoMeta extends Bitrix\Iblock\Template\Functions\FunctionBase
{
        ...
        if($kombox!='Y'){
            ...
            foreach(self::$FilterResult["ITEMS"] as $key => $prop){
                ...
                foreach($props as $id => $val)
                {
                    ...
                    elseif(!isset($val['CHECKED']) && isset($val['VALUE']) && isset($val['HTML_VALUE'])
                        && !empty($val['HTML_VALUE']) && $val['HTML_VALUE'] != $val['VALUE'] && isset($val['RANGE_VALUE']))
                    {
                        $Xvalue[] = $id;
                ...
                if(isset($FilterResult['ITEMS'][$idSection]['VALUES']) && is_array($FilterResult['ITEMS'][$idSection]['VALUES']))
                    foreach($FilterResult['ITEMS'][$idSection]['VALUES'] as $key=>$param)
                    {
                        if((is_int($idCondition) && $key == $idCondition) ||
                            strcmp($param['VALUE'], $idCondition) == 0 || strcmp($param['LIST_VALUE'], $idCondition) == 0 ||
                            /*(isset($param['FACET_VALUE']) && strcmp($param['FACET_VALUE'], $idCondition) == 0) ||*/
                            (strcmp($idSection, 'SECTION_ID') == 0 && strcmp($param['CONTROL_NAME_SEF'], $idCondition) == 0) ||
    ...
    private function CheckElementsGreat($idSection,$idCondition,$type=0)
    {
        ...
        if($type===0)
        {
            ...
            {
                if(
                    isset($FilterResult['ITEM'][$idSection]['MAX']['HTML_VALUE'])
                    &&
                    !empty($FilterResult['ITEMS'][$idSection]['MAX']['HTML_VALUE'])
                )
                    $return=1;
                else
                    $return=0;
        ...
        if($type===0)
        {
            if($idCondition=='')
            {
                if(
                    isset($FilterResult['ITEMS'][$idSection]['MIN']['HTML_VALUE'])
                    &&
                    !empty($FilterResult['ITEMS'][$idSection]['MIN']['HTML_VALUE'])
                )
                    $return=1;
                else
                    $return=0;
