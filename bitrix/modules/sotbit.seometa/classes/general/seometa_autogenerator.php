...
class CSeoMetaAutogenerator
{
    ...
    /* получение всех типов инфоблоков */
    ...
    /* получение всех инфоблоков по типу */
	...
	{
        $arIBlockList = array();
        $rsIBlocks = CCatalog::GetList(array(), array(), false, false, array('IBLOCK_ID'));
        ...
        if(!empty($arIBlockList))
        {
            ...
            foreach($arIBlockList as $intIBlockID)
            {
                ...
                $arrProps = array();
                $rsProps = CIBlockProperty::GetList(array("sort" => "asc", "name" => "asc"), array("ACTIVE" => "Y", "IBLOCK_ID" => $intIBlockID));
    ...
    public function startGeneration($condition)
    {
        ...
        {
            $conditionName = $condition['NAME'] . '_'; // название по умолчанию
            $defaultName = true;
        }
    ...
    private function getAllCombinations($rule, $logic)
    {
        ...
        foreach($arrCombs as $index => $vals)
        {
            ...
            $allCombs[$index] = array(
                'CLASS_ID' => 'CondGroup',
                'DATA' => array('All' => $logic, 'True' => 'True'),
    ...
    // формирование меню для шаблонизатора метаданных
