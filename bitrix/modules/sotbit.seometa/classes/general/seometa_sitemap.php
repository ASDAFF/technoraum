...
class CSeoMetaSitemap extends CCSeoMeta
{
    ...
    private function GetValueOfListProp($ConditionValue, $IdIblock, $IdProperty, $ConditionSections, $PropertyCode)
    {
        ...
        else
        {
            ...
            if($enum_fields = $property_enums->GetNext())
            {
                ...
                $return[$IdProperty]['BITRIX'][1][] = str_replace(' ', '%20', strtolower($enum_fields['XML_ID']));
            ...
            if ($ar_res = $res->GetNext())
            {
                ...
                {                    
                    ...
                    while ( $row = $result->Fetch() )
                    {
                        ...
                        $return[$IdProperty]['BITRIX'][1][] = strtolower($row['UF_XML_ID']);
    ...
    private function GetValueOfTextProp($ConditionValue, $IdIblock, $IdProperty, $ConditionSections, $PropertyCode)
    {
        ...
        else
        {
            ...
            //$return[$IdProperty]['BITRIX'][1][] = str_replace(' ', '%20', strtolower($GLOBALS['APPLICATION']->ConvertCharset($ConditionValue, SITE_CHARSET, 'UTF-8')));
