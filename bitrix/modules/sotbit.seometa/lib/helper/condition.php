...
class Condition
{
    ...
    */
    private function getByKey($newConditions, $key)
    {
        $result = array();
    ...
    /**
     * assemble conditions by condition where there aren't children width group of condition and condition there aren't children
    ...
    */
    private function assembleConditions(array $Condition, array $oldConditions, array $newConditions)
    {
        ...
        if(isset($Condition['DATA']['All']) && $Condition['DATA']['All'] == 'AND')
        {
            $andNew = $this->getByKey($newConditions, 'AND');
            ...
            $newConditions = $this->getByKey($newConditions, 'OR');
        ...
        elseif(isset($Condition['DATA']['All']) && $Condition['DATA']['All'] == 'OR')
        {
            foreach($oldConditions as &$arCond)
                if (isset($arCond['CLASS_ID']))
                    $arCond = array($arCond);

            $orNew = $this->getByKey($newConditions, 'OR');
            ...
            $newConditions = $this->getByKey($newConditions, 'AND');
    ...
    */
    ...
    protected function AllPropertiesByList($IdIblock, $ConditionSections, $IdProperty, $property)
    {
        ...
        {
            if($filterBySection)
                $arFilter += ["SECTION_ID" => $Section, "INCLUDE_SUBSECTIONS" => 'Y'];
