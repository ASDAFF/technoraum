...
class TagWriter extends AbstractWriter
{
    ...
    public function Write(array $arFields)
    {
        ...
        if(in_array($this->arCondition['ID'], $this->WorkingConditions) && unserialize($this->arCondition['CONDITION_TAG']))
        {
            if($arFields['strict_relinking'] != 'Y')
                $Title = \Bitrix\Iblock\Template\Engine::process($sku, unserialize($this->arCondition['CONDITION_TAG']));
            else
