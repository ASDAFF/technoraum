...
$colorScheme = COption::GetOptionString($module_id, "color_scheme", "BRIGHT");
$colorTheme = COption::GetOptionString($module_id, "color_theme", "");
...
$arJQ = Array(
	...
	"2" => GetMessage("ALTASIB_GEOBASE_JQUERY_2"),
...
if(!CModule::IncludeModule("statistic"))
	unset($useSource["statistic"]);

$arYCMode = array(
	"large" => GetMessage("ALTASIB_GEOBASE_LARGE"),
	"small" => GetMessage("ALTASIB_GEOBASE_SMALL"),
);

$arModeLoc = array(
	"cities" => GetMessage("ALTASIB_GEOBASE_MODE_CITIES"),
	"regions" => GetMessage("ALTASIB_GEOBASE_MODE_REGIONS"),
	"all" => GetMessage("ALTASIB_GEOBASE_MODE_ALL")
);
$arTimeRdr = array(
	"N" => GetMessage("ALTASIB_GEOBASE_RDR_NOT_TIME"),
	"session" => GetMessage("ALTASIB_GEOBASE_RDR_SESSIONS"),
	"3600" => GetMessage("ALTASIB_GEOBASE_RDR_HOUR"),
	"86400" => GetMessage("ALTASIB_GEOBASE_RDR_DAY"),
	"604800" => GetMessage("ALTASIB_GEOBASE_RDR_WEEK"),
	"2592000" => GetMessage("ALTASIB_GEOBASE_RDR_MONTH"),
	"31536000" => GetMessage("ALTASIB_GEOBASE_RDR_YEAR")
);

$arColSm = array(
	'BRIGHT' => GetMessage('AGB_SCHEME_BRIGHT'),
	'PALE' => GetMessage('AGB_SCHEME_PALE'),
);

if($colorScheme == "BRIGHT")
{
	$arColors = array(
		"" => GetMessage("AGB_OTHER"),
		"c1" => GetMessage("AGB_SCHEME_GB"),
		"c2" => GetMessage("AGB_SCHEME_GG"),
		"c3" => GetMessage("AGB_SCHEME_R"),
		"c4" => GetMessage("AGB_SCHEME_G"),
		"c5" => GetMessage("AGB_SCHEME_P"),
		"c6" => GetMessage("AGB_SCHEME_O"),
		"c7" => GetMessage("AGB_SCHEME_S"),
		"c8" => GetMessage("AGB_SCHEME_SS"),
	);
}
elseif($colorScheme == "PALE")
{
	$arColors = array(
		"" => GetMessage("AGB_OTHER"),
		"c1" => GetMessage("AGB_SCHEME_GB_L"),
		"c2" => GetMessage("AGB_SCHEME_GG_L"),
		"c3" => GetMessage("AGB_SCHEME_R_L"),
		"c4" => GetMessage("AGB_SCHEME_G_L"),
		"c5" => GetMessage("AGB_SCHEME_P_L"),
		"c6" => GetMessage("AGB_SCHEME_O_L"),
		"c7" => GetMessage("AGB_SCHEME_S_L"),
		"c8" => GetMessage("AGB_SCHEME_SS_L"),
	);
}
...
$arAllOptions = array(
	...
	"data" => Array(
		Array("source", GetMessage("ALTASIB_GEOBASE_SOURCE"), "local_db", array("selectbox", $useSource)),
		Array("ipgeobase_enable", GetMessage("ALTASIB_GEOBASE_IPGEOBASE_ENABLE"), "Y", Array("checkbox")),
		Array("geoip_enable", GetMessage("ALTASIB_GEOBASE_GEOIP_ENABLE"), "Y", Array("checkbox")),
	),
	"services" => Array(
	...
	"auto_display" => Array(
		...
		Array("note" => GetMessage("ALTASIB_GEOBASE_YC_MODE")),
	...
	"redirects" => Array(
		Array("redirect_enable", GetMessage("ALTASIB_GEOBASE_REDIRECT_ENABLE"), "Y", Array("checkbox")),
		Array("redirect_onhit_enable", GetMessage("ALTASIB_GEOBASE_REDIRECT_ONHIT_EN"), "N", Array("checkbox")),
		Array("redirect_time", GetMessage("ALTASIB_GEOBASE_RDR_TIME"), "86400", Array("selectbox", $arTimeRdr)),
		Array("redirect_save_curpage", GetMessage("ALTASIB_GEOBASE_REDIRECT_SAVE_CURPAGE"), "N", Array("checkbox")),
		Array("SPREAD_COOKIE", GetMessage("ALTASIB_GEOBASE_SPREAD_COOKIE"), "Y", Array("checkbox")),
	),
	"colors" => Array(
		Array("color_scheme", GetMessage("ALTASIB_GEOBASE_COLOR_SCHEME"), "Y", Array("selectbox", $arColSm)),
		Array("color_theme", GetMessage("AGB_COLOR_SCHEME"), "86400", Array("selectbox", $arColors)),
	),
	"locations" => Array(
	...
	"user_fields" => Array(
		Array("note" => GetMessage("ALTASIB_GEOBASE_UF_NOTE", array("#ENTITY_ID#" => "ALTASIB_GEOBASE"))),
	),
);

// if(isset($colorTheme) && $colorTheme == "")
	$arAllOptions["colors"][] = Array("color_other", GetMessage("AGB_COLOR_OTHER"), "#0097f6", Array("text"));
	$arAllOptions["colors"][] = Array("note" => GetMessage("ALTASIB_GEOBASE_COLOR_NOTE", array("#ON_CLICK#" => "SetEmptyColorTheme(event)")));

// for each site code
$rsSites = CSite::GetList($by="sort", $order="desc", Array("ACTIVE" => "Y"));
while ($arSite = $rsSites->Fetch())
{
	$arAllOptions["services"][] = Array(
		"geoip_top_code_".$arSite["ID"],
		GetMessage("ALTASIB_GEOBASE_GEOIP_CODE_SITES", Array("#SITE#" => "<b title='".$arSite["SITE_NAME"].($arSite["SERVER_NAME"] ? " [".$arSite["SERVER_NAME"]."]" : "")."'>(".$arSite["ID"].") ".$arSite["NAME"]."</b>")),
		"",
		Array("text")
	);
}
...
{
	$arLocOptions = array();
	$rsPType = CSalePersonType::GetList(Array("SORT" => "ASC"), Array("ACTIVE"=>'Y'));
	while($arPType = $rsPType->Fetch())
	{
		$strPropIdDef = '';

		$resProps = CSaleOrderProps::GetList(
			array("SORT" => "ASC"),
			array(
		...
		{
			$arPropsSB[$arProps["ID"]] = "#".$arProps["ID"]." ".$arProps["NAME"]." [".$arProps["CODE"]."]";
			if(empty($strPropIdDef))
				$strPropIdDef = $arProps["ID"];
		}
		$arPropsSB[""] = GetMessage("ALTASIB_GEOBASE_NOT_SET");

		$arLocOptions[] = Array(
			"field_loc_".$arPType["ID"],
			GetMessage("ALTASIB_GEOBASE_FIELD_LOCPERSON", array("#PERSON_TYPE_ID#" => $arPType["ID"], "#PERSON_TYPE_NAME#" => $arPType["NAME"], "#SITES#" => implode(", ", $arPType["LIDS"]))),
			$strPropIdDef,
			Array("selectbox", $arPropsSB)
		);
	...
	if($bLocs)
	{
		$arAllOptions["locations"][] = Array("location_template", GetMessage("ALTASIB_GEOBASE_FIELD_TEMPLATE"), "ORDER_PROP_", Array("text"));
		$arAllOptions["locations"] = array_merge($arAllOptions["locations"], $arLocOptions);
	}
}

if(!$bLocs)
{
	$arAllOptions["locations"][] = Array("field_loc_ind", GetMessage("ALTASIB_GEOBASE_FIELD_LOC_IND"), "ORDER_PROP_2", Array("text"));
	$arAllOptions["locations"][] = Array("field_loc_leg", GetMessage("ALTASIB_GEOBASE_FIELD_LOC_LEG"), "ORDER_PROP_3", Array("text"));
}
...
<form method="post" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsEx($mid)?>&amp;lang=<?echo LANG?>">
	...
	$strISsMarketHeader = '';
	$strISsMarketHeader = '<div style="background-color:#fff;padding:0;border-top:1px solid #8E8E8E;border-bottom:1px solid #8E8E8E;margin-bottom:15px;">
	...
	elseif ($incMod == '2')
	{
		$strDemoStatus = '<span class="errortext">'.GetMessage("ALTASIB_GEOBASE_DEMO_MODE", Array("#MODULE#" => $module_id)).'</span><br/>';
	}
	elseif ($incMod == '3')
	{
		$strDemoStatus = CAdminMessage::ShowMessage(Array("MESSAGE"=>GetMessage("ALTASIB_GEOBASE_DEMO_EXPIRED", Array("#MODULE#" => $module_id)), "HTML"=>true, "TYPE"=>"ERROR"));
	}
?>
	<tr><td colspan="2"><? echo $strISsMarketHeader; echo $strDemoStatus;?>
		<? $strDemoStatus .="<br>"; ?>
	</td></tr>
	<tr>
		<td colspan="2">
			<div class="notes">
				<table cellspacing="0" cellpadding="0" border="0" class="notes" align="center">
					...
					<tr>
						<td class="left"><div class="empty"></div></td>
						<td class="content"><?echo GetMessage("ALTASIB_GEOBASE_DESCR")?></td>
						<td class="right"><div class="empty"></div></td>
					</tr>
					<tr class="bottom">
						<td class="left"><div class="empty"></div></td>
	...
	<tr class="heading">
		<td colspan="2"><?=GetMessage("ALTASIB_GEOBASE_RDR_SETTS")?></td>
	</tr>
	<?ShowParamsHTMLByArray($arAllOptions["redirects"]);?>

	<tr class="heading">
		<td colspan="2"><?=GetMessage("ALTASIB_GEOBASE_COLOR_SHM")?></td>
	</tr>
	<?ShowParamsHTMLByArray($arAllOptions["colors"]);?>
	...
	<tr><td colspan="2"><?=$strISsMarketHeader?><?=$strDemoStatus?></td></tr>
...
.altasib_gb_uf_edit{
	...
	background-image:url("/bitrix/images/altasib.geobase/correct.gif");
...
$(document).ready(function(){
	...
	if($('input[name=redirect_onhit_enable]').prop('checked'))
		$('select[name=redirect_time]').attr('disabled',false);
	else
		$('select[name=redirect_time]').attr('disabled',true);

	if($('select[name=color_theme]').val() == ""){
		$('input[name=color_other]').attr('disabled',false);
		$('select[name=color_scheme]').attr('disabled',true);
	}else{
		$('input[name=color_other]').attr('disabled',true);
		$('select[name=color_scheme]').attr('disabled',false);
	}
});

$('input[name=redirect_onhit_enable]').change(function(){
...
function SetEmptyColorTheme(e){
	e.preventDefault();
	$('select[name=color_theme]').val('').change();
	var colorInput = $('input[name=color_other]');
	var strLength = colorInput.val().length*2;
	colorInput.focus();
	colorInput[0].setSelectionRange(strLength,strLength);
	return false;
}

$('select[name=color_theme]').change(function(){
	if($(this).val() == ""){
		$('input[name=color_other]').attr('disabled',false);
		$('select[name=color_scheme]').attr('disabled',true);
	}else{
...
function updateDB(dst){
	...
	<tr class="heading" id="GeoIPSettings">
		<td colspan="2"><?=GetMessage("ALTASIB_GEOBASE_GEOIP_SETS")?></td>
	</tr>
	<?ShowParamsHTMLByArray($arAllOptions["services"]);?>

<?	if(CModule::IncludeModule("statistic")):?>
	<tr class="heading" id="GeoLoockup">
		<td colspan="2"><?=GetMessage("ALTASIB_GEOBASE_LOOCKUP")?></td>
	</tr>
...
<?
	...
	if(!empty($dip))
		echo GetMessage("ALTASIB_GEOBASE_SRV_DETECT", array("#IP#" => $dip));
	else
		echo GetMessage("ALTASIB_GEOBASE_SRV_ND");
...
?>
	...
	<tr>
		<td colspan="2" align="center"><h4><?=GetMessage("ALTASIB_GEOBASE_SRV")?></h4></td>
	</tr>
	<tr>
		<td colspan="2" align="center">
<?
	...
	echo BeginNote();
?>
		<table class="internal" id="server_print_arr">
			<tbody>
			<tr width="100%" class="heading">
				<td><?=GetMessage("ALTASIB_GEOBASE_KEY")?></td>
				<td><?=GetMessage("ALTASIB_GEOBASE_VAL")?></td>
			</tr>
		<?
	foreach($_SERVER as $field => $value)
	{
		if(!in_array($field, $arSKeys) || empty($value))
			continue;
...
?>
		...
		<td colspan="2">
			<table class="internal">
				...
				<tr width="100%" class="heading" id="altasib_geobase_table_header">
					<td>ID</td>
					<td><?=GetMessage("ALTASIB_TABLE_SORT");?></td>
					...
					<td><?=GetMessage("ALTASIB_TABLE_CITY_UFIELD");?></td>
	...
	<tr class="heading">
		<td colspan="2"><?=GetMessage("ALTASIB_INP_CITY_ADD")?></td>
	</tr>
	<tr>
		<td>
			<input type="text" size="100" maxlength="255" id="altasib_geobase_search" onkeyup="altasib_geobase_inpKey(event);" autocomplete="off" placeholder="<?=GetMessage("ALTASIB_INP_ENTER_CITY");?>" name="altasib_geobase_search" value="">
			<br/>
	...
	<tr class="heading">
		<td colspan="2"><?=GetMessage("ALTASIB_GEOBASE_UF_LIST")?></td>
	</tr>
	<?ShowParamsHTMLByArray($arAllOptions["user_fields"]);?>
		...
		success: function(data){
			...
			if(data == '' || data == null)
				...
				for(var i=0, len=data.length; i < len; i++){
					var sOVal = (typeof(data[i]['CITY']) == "undefined" || data[i]['CITY'] == null ? '' : data[i]['CITY']);
					sOVal += (typeof(data[i]['REGION']) == "undefined" || data[i]['REGION'] == null ? '' : (sOVal.length>0? ', ':'') + data[i]['REGION']);
					sOVal += (typeof(data[i]['DISTRICT']) == "undefined" || BX.util.trim(data[i]['DISTRICT']) == '' || data[i]['DISTRICT'] == null ? '' : (sOVal.length>0? ', ':'') + data[i]['DISTRICT']);
					sOVal += (typeof(data[i]['COUNTRY']) == "undefined" || BX.util.trim(data[i]['COUNTRY']) == '' ? '' : (sOVal.length>0? ', ':'') + data[i]['COUNTRY']);
					arOut += '<option id="altasib_geobase_inp_'+ (typeof(data[i]['C_CODE']) == "undefined" ? data[i]['ID'] : data[i]['C_CODE']) +'"'
					+'value = "'+ sOVal +'">'+ sOVal +'</option>\n';
				}
				list.html(arOut);
...
function altasib_geobase_uf(id){
	var popup = new BX.CDialog({
		'title': '<?=GetMessage("ALTASIB_GEOBASE_EDT_UF");?>',
		'content_url': '/bitrix/admin/altasib_geobase_uf.php',
		'content_post': {
...
?><?$tabControl->BeginNextTab();?>
	<tr><td colspan="2"><?echo $strISsMarketHeader; echo $strDemoStatus;?></td></tr>
<?
	require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/admin/group_rights.php");
	$tabControl->Buttons();?>
	<script language="JavaScript">
