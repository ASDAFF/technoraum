...
Class CAltasibGeoBaseImport
{
	...
	{
		$arFiles = array('Kladr_districts.csv', 'Kladr_region.csv', 'Kladr_selected.csv', 'Kladr_cities.csv');
		...
		for($i=0; $i<count($Tables); $i++){
			if(ToLower(SITE_CHARSET) == "utf-8"){
				$sFName = $_SERVER['DOCUMENT_ROOT'].$UPath.$arFiles[$i];
				file_put_contents($sFName, iconv("windows-1251", "UTF-8", file_get_contents($sFName)));
			}
			CAltasibGeoBaseImport::InsertDataTable($UPath.$arFiles[$i], $Tables[$i], $aFields[$i]);
		}
	...
	{
		global $DB;
		$arFiles = array('geo_city.csv', 'region-code-localized.csv', 'geo_country.csv');
		...
		for($i=0; $i<count($Tables); $i++){
			if(ToLower(SITE_CHARSET) == "utf-8" && $Tables[$i] != "altasib_geobase_mm_region"){
				$sFName = $_SERVER['DOCUMENT_ROOT'].$UPath.$arFiles[$i];
				file_put_contents($sFName, iconv("windows-1251", "UTF-8", file_get_contents($sFName)));
			}
			CAltasibGeoBaseImport::InsertDataTable($UPath.$arFiles[$i], $Tables[$i], $aFields[$i]);
	...
	function InsertDataTable($flName, $table, $afields)
	{
		global $DB;
		$table = $DB->ForSQL($table);
		$sFName = $_SERVER['DOCUMENT_ROOT'].$flName;
		$cols = "(".implode(',', $afields).")";
		$handle = fopen($sFName, "r");
